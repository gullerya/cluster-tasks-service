...
final class PostgreSqlDbDataProvider extends ClusterTasksDbDataProvider {
	...
	PostgreSqlDbDataProvider(ClusterTasksService clusterTasksService, ClusterTasksServiceConfigurerSPI serviceConfigurer) {
		...
		insertSelfLastSeenSQL = "INSERT INTO " + ACTIVE_NODES_TABLE_NAME + " (" + ACTIVE_NODE_ID + "," + ACTIVE_NODE_SINCE + "," + ACTIVE_NODE_LAST_SEEN + ")" + " VALUES (?, TIMEZONE('UTC', NOW()), TIMEZONE('UTC', NOW()))";
		updateSelfLastSeenSQL = "UPDATE " + ACTIVE_NODES_TABLE_NAME + " SET " + ACTIVE_NODE_LAST_SEEN + " = TIMEZONE('UTC', NOW()) WHERE " + ACTIVE_NODE_ID + " = ?";
		removeLongTimeNoSeeSQL = "DELETE FROM " + ACTIVE_NODES_TABLE_NAME + " WHERE " + ACTIVE_NODE_LAST_SEEN + " < TIMEZONE('UTC', NOW()) - MAKE_INTERVAL(SECS := ? / 1000)";

		//  insert / update tasks
		insertTaskSQL = "SELECT insert_task(" + String.join(",", Collections.nCopies(9, "?")) + ")";
		updateScheduledTaskIntervalSQL = "UPDATE " + META_TABLE_NAME +
				" SET " + CREATED + " = TIMEZONE('UTC', NOW()), " + DELAY_BY_MILLIS + " = ?" +
		...
		for (int maxProcessorTypes : new Integer[]{20, 50, 100, 500}) {
			...
			selectForUpdateTasksSQLs.put(maxProcessorTypes,
					"SELECT * FROM" +
							...
							"       AND " + CREATED + " < TIMEZONE('UTC', NOW()) - MAKE_INTERVAL(SECS := " + DELAY_BY_MILLIS + " / 1000)) meta" +
		...
		updateTasksStartedSQL = "UPDATE " + META_TABLE_NAME + " SET " + STATUS + " = " + ClusterTaskStatus.RUNNING.value + ", " + STARTED + " = TIMEZONE('UTC', NOW()), " + RUNTIME_INSTANCE + " = ?" +
				" WHERE " + META_ID + " = ?";
